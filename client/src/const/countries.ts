export const Country = {
  Algeria: { code: "DZ", ianaTimezone: "Africa/Algiers" },
  Angola: { code: "AO", ianaTimezone: "Africa/Lagos" },
  Argentina: { code: "AR", ianaTimezone: "America/Argentina/Buenos_Aires" },
  Australia: { code: "AU", ianaTimezone: "Australia/Sydney" },
  Austria: { code: "AT", ianaTimezone: "Europe/Vienna" },
  Bahrain: { code: "BH", ianaTimezone: "Asia/Qatar" },
  Bangladesh: { code: "BD", ianaTimezone: "Asia/Dhaka" },
  Belgium: { code: "BE", ianaTimezone: "Europe/Brussels" },
  "Bosnia and Herzegovina": { code: "BA", ianaTimezone: "Europe/Belgrade" },
  Botswana: { code: "BW", ianaTimezone: "Africa/Maputo" },
  Bulgaria: { code: "BG", ianaTimezone: "Europe/Sofia" },
  Brazil: { code: "BR", ianaTimezone: "America/Sao_Paulo" },
  Canada: { code: "CA", ianaTimezone: "America/Toronto" },
  Chile: { code: "CL", ianaTimezone: "America/Santiago" },
  Colombia: { code: "CO", ianaTimezone: "America/Bogota" },
  "Costa Rica": { code: "CR", ianaTimezone: "America/Costa_Rica" },
  Croatia: { code: "HR", ianaTimezone: "Europe/Belgrade" },
  Cyprus: { code: "CY", ianaTimezone: "Asia/Nicosia" },
  Czechia: { code: "CZ", ianaTimezone: "Europe/Prague" },
  Denmark: { code: "DK", ianaTimezone: "Europe/Copenhagen" },
  "Dominican Republic": { code: "DO", ianaTimezone: "America/Santo_Domingo" },
  Ecuador: { code: "EC", ianaTimezone: "Pacific/Galapagos" },
  "El Salvador": { code: "SV", ianaTimezone: "America/El_Salvador" },
  Egypt: { code: "EG", ianaTimezone: "Africa/Cairo" },
  Estonia: { code: "EE", ianaTimezone: "Europe/Tallinn" },
  Finland: { code: "FI", ianaTimezone: "Europe/Helsinki" },
  France: { code: "FR", ianaTimezone: "Europe/Paris" },
  Germany: { code: "DE", ianaTimezone: "Europe/Berlin" },
  Greece: { code: "GR", ianaTimezone: "Europe/Athens" },
  Guatemala: { code: "GT", ianaTimezone: "America/Guatemala" },
  Honduras: { code: "HN", ianaTimezone: "America/Tegucigalpa" },
  "Hong Kong SAR": { code: "HK", ianaTimezone: "Asia/Hong_Kong" },
  Hungary: { code: "HU", ianaTimezone: "Europe/Budapest" },
  Iceland: { code: "IS", ianaTimezone: "Atlantic/Reykjavik" },
  Ireland: { code: "IE", ianaTimezone: "Europe/Dublin" },
  Indonesia: { code: "ID", ianaTimezone: "Asia/Jakarta" },
  India: { code: "IN", ianaTimezone: "Asia/Calcutta" },
  Italy: { code: "IT", ianaTimezone: "Europe/Rome" },
  Jamaica: { code: "JM", ianaTimezone: "America/Jamaica" },
  Japan: { code: "JP", ianaTimezone: "Asia/Tokyo" },
  Kenya: { code: "KE", ianaTimezone: "Africa/Nairobi" },
  Latvia: { code: "LV", ianaTimezone: "Europe/Riga" },
  Lebanon: { code: "LB", ianaTimezone: "Asia/Beirut" },
  Lithuania: { code: "LT", ianaTimezone: "Europe/Vilnius" },
  Luxembourg: { code: "LU", ianaTimezone: "Europe/Luxembourg" },
  Malaysia: { code: "MY", ianaTimezone: "Asia/Kuala_Lumpur" },
  Maldives: { code: "MV", ianaTimezone: "Indian/Maldives" },
  Malta: { code: "MT", ianaTimezone: "Europe/Malta" },
  Mauritius: { code: "MU", ianaTimezone: "Indian/Mauritius" },
  Mexico: { code: "MX", ianaTimezone: "America/Mexico_City" },
  Moldova: { code: "MD", ianaTimezone: "Europe/Chisinau" },
  Morocco: { code: "MA", ianaTimezone: "Africa/Casablanca" },
  Netherlands: { code: "NL", ianaTimezone: "Europe/Amsterdam" },
  "New Zealand": { code: "NZ", ianaTimezone: "Pacific/Auckland" },
  Nicaragua: { code: "NI", ianaTimezone: "America/Managua" },
  Nigeria: { code: "NG", ianaTimezone: "Africa/Lagos" },
  Norway: { code: "NO", ianaTimezone: "Europe/Oslo" },
  Panama: { code: "PA", ianaTimezone: "America/Panama" },
  Paraguay: { code: "PY", ianaTimezone: "America/Asuncion" },
  Peru: { code: "PE", ianaTimezone: "America/Lima" },
  Philippines: { code: "PH", ianaTimezone: "Asia/Manila" },
  Poland: { code: "PL", ianaTimezone: "Europe/Warsaw" },
  Portugal: { code: "PT", ianaTimezone: "Europe/Lisbon" },
  "Puerto Rico": { code: "PR", ianaTimezone: "America/Puerto_Rico" },
  Qatar: { code: "QA", ianaTimezone: "Asia/Qatar" },
  Romania: { code: "RO", ianaTimezone: "Europe/Bucharest" },
  Russia: { code: "RF", ianaTimezone: "Europe/Moscow" },
  "Saudi Arabia": { code: "SA", ianaTimezone: "Asia/Riyadh" },
  Serbia: { code: "RS", ianaTimezone: "Europe/Belgrade" },
  Singapore: { code: "SG", ianaTimezone: "Asia/Singapore" },
  Slovakia: { code: "SK", ianaTimezone: "Europe/Prague" },
  Slovenia: { code: "SI", ianaTimezone: "Europe/Belgrade" },
  "South Africa": { code: "ZA", ianaTimezone: "Africa/Johannesburg" },
  "South Korea": { code: "KR", ianaTimezone: "Asia/Seoul" },
  Spain: { code: "ES", ianaTimezone: "Europe/Madrid" },
  "Sri Lanka": { code: "LK", ianaTimezone: "Asia/Colombo" },
  Sweden: { code: "SE", ianaTimezone: "Europe/Stockholm" },
  Switzerland: { code: "CH", ianaTimezone: "Europe/Zurich" },
  Taiwan: { code: "TW", ianaTimezone: "Asia/Taipei" },
  Thailand: { code: "TH", ianaTimezone: "Asia/Bangkok" },
  "Trinidad and Tobago": { code: "TT", ianaTimezone: "America/Port_of_Spain" },
  Turkey: { code: "TR", ianaTimezone: "Europe/Istanbul" },
  Ukraine: { code: "UA", ianaTimezone: "Europe/Kiev" },
  "United Arab Emirates": { code: "AE", ianaTimezone: "Asia/Dubai" },
  "United Kingdom": { code: "GB", ianaTimezone: "Europe/London" },
  "United States": { code: "US", ianaTimezone: "America/New_York" },
  Uruguay: { code: "UY", ianaTimezone: "America/Montevideo" },
  Vietnam: { code: "VN", ianaTimezone: "Asia/Saigon" },
  Zimbabwe: { code: "ZW", ianaTimezone: "Africa/Maputo" },
};

export const TIMEZONES = [
  {
    countryCode: "AD",
    locales: [
      {
        ianaTimezone: "Europe/Andorra",
        name: "(GMT+01:00) Andorra",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "AE",
    locales: [
      {
        ianaTimezone: "Asia/Dubai",
        name: "(GMT+04:00) Dubai",
        data: "6o",
      },
    ],
  },
  {
    countryCode: "AF",
    locales: [
      {
        ianaTimezone: "Asia/Kabul",
        name: "(GMT+04:30) Kabul",
        data: "7i",
      },
    ],
  },
  {
    countryCode: "AG",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "AI",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "AL",
    locales: [
      {
        ianaTimezone: "Europe/Tirane",
        name: "(GMT+01:00) Tirane",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "AM",
    locales: [
      {
        ianaTimezone: "Asia/Yerevan",
        name: "(GMT+04:00) Yerevan",
        data: "6o",
      },
    ],
  },
  {
    countryCode: "AO",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "AQ",
    locales: [
      {
        ianaTimezone: "Antarctica/Palmer",
        name: "(GMT-03:00) Palmer",
        data: "-50{eipx0{-6o{elj2o{-50{etyd0{-6o{ewrio{-50{f56t0{-6o{f7zyo{-50{fgf90{-6o{fj8eo{-50{frnp0{-6o{fuguo{-50{g2w50{-6o{g5x2o{-50",
      },
      {
        ianaTimezone: "Antarctica/Rothera",
        name: "(GMT-03:00) Rothera",
        data: "-50",
      },
      {
        ianaTimezone: "Antarctica/Syowa",
        name: "(GMT+03:00) Syowa",
        data: "50",
      },
      {
        ianaTimezone: "Antarctica/Mawson",
        name: "(GMT+05:00) Mawson",
        data: "8c",
      },
      {
        ianaTimezone: "Antarctica/Vostok",
        name: "(GMT+06:00) Vostok",
        data: "a0",
      },
      {
        ianaTimezone: "Antarctica/Davis",
        name: "(GMT+07:00) Davis",
        data: "bo",
      },
      {
        ianaTimezone: "Antarctica/DumontDUrville",
        name: "(GMT+10:00) Dumont D'Urville",
        data: "go",
      },
      {
        ianaTimezone: "Antarctica/Casey",
        name: "(GMT+11:00) Casey",
        data: "dc{enn6o{ic",
      },
      {
        ianaTimezone: "Pacific/Auckland",
        name: "(GMT+13:00) Auckland",
        data: "lo{e667c{k0{ebknc{lo{ehenc{k0{emt3c{lo{esn3c{k0{ey1jc{lo{f3vjc{k0{f9hrc{lo{ffbrc{k0{fkq7c{lo{fqk7c{k0{fvync{lo{g1snc{k0{g773c{lo",
      },
    ],
  },
  {
    countryCode: "AR",
    locales: [
      {
        ianaTimezone: "America/Argentina/Buenos_Aires",
        name: "(GMT-03:00) Buenos Aires",
        data: "-50",
      },
    ],
  },
  {
    countryCode: "AS",
    locales: [
      {
        ianaTimezone: "Pacific/Pago_Pago",
        name: "(GMT-11:00) Pago Pago",
        data: "-ic",
      },
    ],
  },
  {
    countryCode: "AT",
    locales: [
      {
        ianaTimezone: "Europe/Vienna",
        name: "(GMT+01:00) Vienna",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "AU",
    locales: [
      {
        ianaTimezone: "Australia/Perth",
        name: "(GMT+08:00) Western Time - Perth",
        data: "dc",
      },
      {
        ianaTimezone: "Australia/Adelaide",
        name: "(GMT+10:30) Central Time - Adelaide",
        data: "hi{e66bi{fu{ebsji{hi{eheri{fu{en0zi{hi{esn7i{fu{ey9fi{hi{f3vni{fu{f9pni{hi{ffbvi{fu{fky3i{hi{fqkbi{fu{fw6ji{hi{g1sri{fu{g7ezi{hi",
      },
      {
        ianaTimezone: "Australia/Darwin",
        name: "(GMT+09:30) Central Time - Darwin",
        data: "fu",
      },
      {
        ianaTimezone: "Australia/Brisbane",
        name: "(GMT+10:00) Eastern Time - Brisbane",
        data: "go",
      },
      {
        ianaTimezone: "Australia/Hobart",
        name: "(GMT+11:00) Eastern Time - Hobart",
        data: "ic{e66ao{go{ebsio{ic{eheqo{go{en0yo{ic{esn6o{go{ey9eo{ic{f3vmo{go{f9pmo{ic{ffbuo{go{fky2o{ic{fqkao{go{fw6io{ic{g1sqo{go{g7eyo{ic",
      },
      {
        ianaTimezone: "Australia/Sydney",
        name: "(GMT+11:00) Eastern Time - Melbourne, Sydney",
        data: "ic{e66ao{go{ebsio{ic{eheqo{go{en0yo{ic{esn6o{go{ey9eo{ic{f3vmo{go{f9pmo{ic{ffbuo{go{fky2o{ic{fqkao{go{fw6io{ic{g1sqo{go{g7eyo{ic",
      },
    ],
  },
  {
    countryCode: "AW",
    locales: [
      {
        ianaTimezone: "America/Curacao",
        name: "(GMT-04:00) Curacao",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "AX",
    locales: [
      {
        ianaTimezone: "Europe/Helsinki",
        name: "(GMT+02:00) Helsinki",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "AZ",
    locales: [
      {
        ianaTimezone: "Asia/Baku",
        name: "(GMT+04:00) Baku",
        data: "6o{e5yw0{8c{ecg80{6o",
      },
    ],
  },
  {
    countryCode: "BA",
    locales: [
      {
        ianaTimezone: "Europe/Belgrade",
        name: "(GMT+01:00) Central European Time - Belgrade",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "BB",
    locales: [
      {
        ianaTimezone: "America/Barbados",
        name: "(GMT-04:00) Barbados",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "BD",
    locales: [
      {
        ianaTimezone: "Asia/Dhaka",
        name: "(GMT+06:00) Dhaka",
        data: "a0",
      },
    ],
  },
  {
    countryCode: "BE",
    locales: [
      {
        ianaTimezone: "Europe/Brussels",
        name: "(GMT+01:00) Brussels",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "BF",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "BG",
    locales: [
      {
        ianaTimezone: "Europe/Sofia",
        name: "(GMT+02:00) Sofia",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "BH",
    locales: [
      {
        ianaTimezone: "Asia/Qatar",
        name: "(GMT+03:00) Qatar",
        data: "50",
      },
    ],
  },
  {
    countryCode: "BI",
    locales: [
      {
        ianaTimezone: "Africa/Maputo",
        name: "(GMT+02:00) Maputo",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "BJ",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "BM",
    locales: [
      {
        ianaTimezone: "Atlantic/Bermuda",
        name: "(GMT-04:00) Bermuda",
        data: "-6o{e5bu0{-50{eco8c{-6o{egs20{-50{eo4gc{-6o{es0i0{-50{ezcwc{-6o{f38y0{-50{falcc{-6o{fehe0{-50{fltsc{-6o{fppu0{-50{fx28c{-6o{g1620{-50{g8igc{-6o",
      },
    ],
  },
  {
    countryCode: "BN",
    locales: [
      {
        ianaTimezone: "Asia/Brunei",
        name: "(GMT+08:00) Brunei",
        data: "dc",
      },
    ],
  },
  {
    countryCode: "BO",
    locales: [
      {
        ianaTimezone: "America/La_Paz",
        name: "(GMT-04:00) La Paz",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "BQ",
    locales: [
      {
        ianaTimezone: "America/Curacao",
        name: "(GMT-04:00) Curacao",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "BR",
    locales: [
      {
        ianaTimezone: "America/Rio_Branco",
        name: "(GMT-05:00) Rio Branco",
        data: "-8c",
      },
      {
        ianaTimezone: "America/Boa_Vista",
        name: "(GMT-04:00) Boa Vista",
        data: "-6o",
      },
      {
        ianaTimezone: "America/Campo_Grande",
        name: "(GMT-03:00) Campo Grande",
        data: "-50{e4w50{-6o{ec8mo{-50{eg4l0{-6o{enh2o{-50{erd10{-6o{eypio{-50{f2lh0{-6o{fa5qo{-50{fdtx0{-6o{fle6o{-50{fp2d0{-6o{fwmmo{-50{g0il0{-6o{g7v2o{-50",
      },
      {
        ianaTimezone: "America/Cuiaba",
        name: "(GMT-03:00) Cuiaba",
        data: "-50{e4w50{-6o{ec8mo{-50{eg4l0{-6o{enh2o{-50{erd10{-6o{eypio{-50{f2lh0{-6o{fa5qo{-50{fdtx0{-6o{fle6o{-50{fp2d0{-6o{fwmmo{-50{g0il0{-6o{g7v2o{-50",
      },
      {
        ianaTimezone: "America/Manaus",
        name: "(GMT-04:00) Manaus",
        data: "-6o",
      },
      {
        ianaTimezone: "America/Porto_Velho",
        name: "(GMT-04:00) Porto Velho",
        data: "-6o",
      },
      {
        ianaTimezone: "America/Araguaina",
        name: "(GMT-03:00) Araguaina",
        data: "-50",
      },
      {
        ianaTimezone: "America/Bahia",
        name: "(GMT-03:00) Salvador",
        data: "-50",
      },
      {
        ianaTimezone: "America/Belem",
        name: "(GMT-03:00) Belem",
        data: "-50",
      },
      {
        ianaTimezone: "America/Fortaleza",
        name: "(GMT-03:00) Fortaleza",
        data: "-50",
      },
      {
        ianaTimezone: "America/Maceio",
        name: "(GMT-03:00) Maceio",
        data: "-50",
      },
      {
        ianaTimezone: "America/Recife",
        name: "(GMT-03:00) Recife",
        data: "-50",
      },
      {
        ianaTimezone: "America/Sao_Paulo",
        name: "(GMT-02:00) Sao Paulo",
        data: "-3c{e4w3c{-50{ec8l0{-3c{eg4jc{-50{enh10{-3c{erczc{-50{eyph0{-3c{f2lfc{-50{fa5p0{-3c{fdtvc{-50{fle50{-3c{fp2bc{-50{fwml0{-3c{g0ijc{-50{g7v10{-3c",
      },
      {
        ianaTimezone: "America/Noronha",
        name: "(GMT-02:00) Noronha",
        data: "-3c",
      },
    ],
  },
  {
    countryCode: "BS",
    locales: [
      {
        ianaTimezone: "America/Nassau",
        name: "(GMT-05:00) Nassau",
        data: "-8c{e5bvo{-6o{ecoa0{-8c{egs3o{-6o{eo4i0{-8c{es0jo{-6o{ezcy0{-8c{f38zo{-6o{fale0{-8c{fehfo{-6o{fltu0{-8c{fppvo{-6o{fx2a0{-8c{g163o{-6o{g8ii0{-8c",
      },
    ],
  },
  {
    countryCode: "BT",
    locales: [
      {
        ianaTimezone: "Asia/Thimphu",
        name: "(GMT+06:00) Thimphu",
        data: "a0",
      },
    ],
  },
  {
    countryCode: "BV",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "BW",
    locales: [
      {
        ianaTimezone: "Africa/Maputo",
        name: "(GMT+02:00) Maputo",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "BY",
    locales: [
      {
        ianaTimezone: "Europe/Minsk",
        name: "(GMT+03:00) Minsk",
        data: "50",
      },
    ],
  },
  {
    countryCode: "BZ",
    locales: [
      {
        ianaTimezone: "America/Belize",
        name: "(GMT-06:00) Belize",
        data: "-a0",
      },
    ],
  },
  {
    countryCode: "CA",
    locales: [
      {
        ianaTimezone: "America/Vancouver",
        name: "(GMT-08:00) Pacific Time - Vancouver",
        data: "-dc{e5c0o{-bo{ecof0{-dc{egs8o{-bo{eo4n0{-dc{es0oo{-bo{ezd30{-dc{f394o{-bo{falj0{-dc{fehko{-bo{fltz0{-dc{fpq0o{-bo{fx2f0{-dc{g168o{-bo{g8in0{-dc",
      },
      {
        ianaTimezone: "America/Whitehorse",
        name: "(GMT-08:00) Pacific Time - Whitehorse",
        data: "-dc{e5c0o{-bo{ecof0{-dc{egs8o{-bo{eo4n0{-dc{es0oo{-bo{ezd30{-dc{f394o{-bo{falj0{-dc{fehko{-bo{fltz0{-dc{fpq0o{-bo{fx2f0{-dc{g168o{-bo{g8in0{-dc",
      },
      {
        ianaTimezone: "America/Dawson_Creek",
        name: "(GMT-07:00) Mountain Time - Dawson Creek",
        data: "-bo",
      },
      {
        ianaTimezone: "America/Edmonton",
        name: "(GMT-07:00) Mountain Time - Edmonton",
        data: "-bo{e5bz0{-a0{ecodc{-bo{egs70{-a0{eo4lc{-bo{es0n0{-a0{ezd1c{-bo{f3930{-a0{falhc{-bo{fehj0{-a0{fltxc{-bo{fppz0{-a0{fx2dc{-bo{g1670{-a0{g8ilc{-bo",
      },
      {
        ianaTimezone: "America/Yellowknife",
        name: "(GMT-07:00) Mountain Time - Yellowknife",
        data: "-bo{e5bz0{-a0{ecodc{-bo{egs70{-a0{eo4lc{-bo{es0n0{-a0{ezd1c{-bo{f3930{-a0{falhc{-bo{fehj0{-a0{fltxc{-bo{fppz0{-a0{fx2dc{-bo{g1670{-a0{g8ilc{-bo",
      },
      {
        ianaTimezone: "America/Regina",
        name: "(GMT-06:00) Central Time - Regina",
        data: "-a0",
      },
      {
        ianaTimezone: "America/Winnipeg",
        name: "(GMT-06:00) Central Time - Winnipeg",
        data: "-a0{e5bxc{-8c{ecobo{-a0{egs5c{-8c{eo4jo{-a0{es0lc{-8c{ezczo{-a0{f391c{-8c{falfo{-a0{fehhc{-8c{fltvo{-a0{fppxc{-8c{fx2bo{-a0{g165c{-8c{g8ijo{-a0",
      },
      {
        ianaTimezone: "America/Iqaluit",
        name: "(GMT-05:00) Eastern Time - Iqaluit",
        data: "-8c{e5bvo{-6o{ecoa0{-8c{egs3o{-6o{eo4i0{-8c{es0jo{-6o{ezcy0{-8c{f38zo{-6o{fale0{-8c{fehfo{-6o{fltu0{-8c{fppvo{-6o{fx2a0{-8c{g163o{-6o{g8ii0{-8c",
      },
      {
        ianaTimezone: "America/Toronto",
        name: "(GMT-05:00) Eastern Time - Toronto",
        data: "-8c{e5bvo{-6o{ecoa0{-8c{egs3o{-6o{eo4i0{-8c{es0jo{-6o{ezcy0{-8c{f38zo{-6o{fale0{-8c{fehfo{-6o{fltu0{-8c{fppvo{-6o{fx2a0{-8c{g163o{-6o{g8ii0{-8c",
      },
      {
        ianaTimezone: "America/Halifax",
        name: "(GMT-04:00) Atlantic Time - Halifax",
        data: "-6o{e5bu0{-50{eco8c{-6o{egs20{-50{eo4gc{-6o{es0i0{-50{ezcwc{-6o{f38y0{-50{falcc{-6o{fehe0{-50{fltsc{-6o{fppu0{-50{fx28c{-6o{g1620{-50{g8igc{-6o",
      },
      {
        ianaTimezone: "America/St_Johns",
        name: "(GMT-03:30) Newfoundland Time - St. Johns",
        data: "-5u{e5bt6{-46{eco7i{-5u{egs16{-46{eo4fi{-5u{es0h6{-46{ezcvi{-5u{f38x6{-46{falbi{-5u{fehd6{-46{fltri{-5u{fppt6{-46{fx27i{-5u{g1616{-46{g8ifi{-5u",
      },
    ],
  },
  {
    countryCode: "CC",
    locales: [
      {
        ianaTimezone: "Indian/Cocos",
        name: "(GMT+06:30) Cocos",
        data: "au",
      },
    ],
  },
  {
    countryCode: "CD",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
      {
        ianaTimezone: "Africa/Maputo",
        name: "(GMT+02:00) Maputo",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "CF",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "CG",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "CH",
    locales: [
      {
        ianaTimezone: "Europe/Zurich",
        name: "(GMT+01:00) Zurich",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "CI",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "CK",
    locales: [
      {
        ianaTimezone: "Pacific/Rarotonga",
        name: "(GMT-10:00) Rarotonga",
        data: "-go",
      },
    ],
  },
  {
    countryCode: "CL",
    locales: [
      {
        ianaTimezone: "Pacific/Easter",
        name: "(GMT-05:00) Easter Island",
        data: "-8c{eipx0{-a0{elj2o{-8c{etyd0{-a0{ewrio{-8c{f56t0{-a0{f7zyo{-8c{fgf90{-a0{fj8eo{-8c{frnp0{-a0{fuguo{-8c{g2w50{-a0{g5x2o{-8c",
      },
      {
        ianaTimezone: "America/Santiago",
        name: "(GMT-03:00) Santiago",
        data: "-50{eipx0{-6o{elj2o{-50{etyd0{-6o{ewrio{-50{f56t0{-6o{f7zyo{-50{fgf90{-6o{fj8eo{-50{frnp0{-6o{fuguo{-50{g2w50{-6o{g5x2o{-50",
      },
    ],
  },
  {
    countryCode: "CM",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "CN",
    locales: [
      {
        ianaTimezone: "Asia/Shanghai",
        name: "(GMT+08:00) China Time - Beijing",
        data: "dc",
      },
    ],
  },
  {
    countryCode: "CO",
    locales: [
      {
        ianaTimezone: "America/Bogota",
        name: "(GMT-05:00) Bogota",
        data: "-8c",
      },
    ],
  },
  {
    countryCode: "CR",
    locales: [
      {
        ianaTimezone: "America/Costa_Rica",
        name: "(GMT-06:00) Costa Rica",
        data: "-a0",
      },
    ],
  },
  {
    countryCode: "CU",
    locales: [
      {
        ianaTimezone: "America/Havana",
        name: "(GMT-05:00) Havana",
        data: "-8c{e5bsc{-6o{eco8c{-8c{egs0c{-6o{eo4gc{-8c{es0gc{-6o{ezcwc{-8c{f38wc{-6o{falcc{-8c{fehcc{-6o{fltsc{-8c{fppsc{-6o{fx28c{-8c{g160c{-6o{g8igc{-8c",
      },
    ],
  },
  {
    countryCode: "CV",
    locales: [
      {
        ianaTimezone: "Atlantic/Cape_Verde",
        name: "(GMT-01:00) Cape Verde",
        data: "-1o",
      },
    ],
  },
  {
    countryCode: "CW",
    locales: [
      {
        ianaTimezone: "America/Curacao",
        name: "(GMT-04:00) Curacao",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "CX",
    locales: [
      {
        ianaTimezone: "Indian/Christmas",
        name: "(GMT+07:00) Christmas",
        data: "bo",
      },
    ],
  },
  {
    countryCode: "CY",
    locales: [
      {
        ianaTimezone: "Asia/Nicosia",
        name: "(GMT+02:00) Nicosia",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "CZ",
    locales: [
      {
        ianaTimezone: "Europe/Prague",
        name: "(GMT+01:00) Central European Time - Prague",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "DE",
    locales: [
      {
        ianaTimezone: "Europe/Berlin",
        name: "(GMT+01:00) Berlin",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "DJ",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "DK",
    locales: [
      {
        ianaTimezone: "Europe/Copenhagen",
        name: "(GMT+01:00) Copenhagen",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "DM",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "DO",
    locales: [
      {
        ianaTimezone: "America/Santo_Domingo",
        name: "(GMT-04:00) Santo Domingo",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "DZ",
    locales: [
      {
        ianaTimezone: "Africa/Algiers",
        name: "(GMT+01:00) Algiers",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "EC",
    locales: [
      {
        ianaTimezone: "Pacific/Galapagos",
        name: "(GMT-06:00) Galapagos",
        data: "-a0",
      },
      {
        ianaTimezone: "America/Guayaquil",
        name: "(GMT-05:00) Guayaquil",
        data: "-8c",
      },
    ],
  },
  {
    countryCode: "EE",
    locales: [
      {
        ianaTimezone: "Europe/Tallinn",
        name: "(GMT+02:00) Tallinn",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "EG",
    locales: [
      {
        ianaTimezone: "Africa/Cairo",
        name: "(GMT+02:00) Cairo",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "EH",
    locales: [
      {
        ianaTimezone: "Africa/El_Aaiun",
        name: "(GMT+00:00) El Aaiun",
        data: "0{e5yzc{1o{e8cjc{0{e9ffc{1o{ecgbc{0{eh7fc{1o{ejd7c{0{ekg3c{1o{enwjc{0{esfvc{1o{eu63c{0{evgrc{1o{ez4zc{0{f3obc{1o{f56rc{0{f69nc{1o{fadfc{0{ff4jc{1o{fg7fc{0{fhabc{1o{fllvc{0{fqczc{1o{fr0bc{0{fs37c{1o{fwubc{0{g1lfc{1o{g20zc{0{g33vc{1o{g8ajc{0",
      },
    ],
  },
  {
    countryCode: "ER",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "ES",
    locales: [
      {
        ianaTimezone: "Atlantic/Canary",
        name: "(GMT+00:00) Canary Islands",
        data: "0{e5yxo{1o{ecg9o{0{eh7do{1o{enwho{0{esfto{1o{ez4xo{0{f3o9o{1o{faddo{0{ff4ho{1o{fllto{0{fqcxo{1o{fwu9o{0{g1ldo{1o{g8aho{0",
      },
      {
        ianaTimezone: "Africa/Ceuta",
        name: "(GMT+01:00) Ceuta",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
      {
        ianaTimezone: "Europe/Madrid",
        name: "(GMT+01:00) Madrid",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "ET",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "FI",
    locales: [
      {
        ianaTimezone: "Europe/Helsinki",
        name: "(GMT+02:00) Helsinki",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "FJ",
    locales: [
      {
        ianaTimezone: "Pacific/Fiji",
        name: "(GMT+13:00) Fiji",
        data: "lo{e3snc{k0{ecnjc{lo{ef13c{k0{eo3rc{lo{eq9jc{k0{ezc7c{lo{f1prc{k0{faknc{lo{fcy7c{k0{flt3c{lo{fo6nc{k0{fx1jc{lo{fzf3c{k0{g8hrc{lo",
      },
    ],
  },
  {
    countryCode: "FK",
    locales: [
      {
        ianaTimezone: "Atlantic/Stanley",
        name: "(GMT-03:00) Stanley",
        data: "-50",
      },
    ],
  },
  {
    countryCode: "FM",
    locales: [
      {
        ianaTimezone: "Pacific/Chuuk",
        name: "(GMT+10:00) Truk",
        data: "go",
      },
      {
        ianaTimezone: "Pacific/Kosrae",
        name: "(GMT+11:00) Kosrae",
        data: "ic",
      },
      {
        ianaTimezone: "Pacific/Pohnpei",
        name: "(GMT+11:00) Ponape",
        data: "ic",
      },
    ],
  },
  {
    countryCode: "FO",
    locales: [
      {
        ianaTimezone: "Atlantic/Faroe",
        name: "(GMT+00:00) Faeroe",
        data: "0{e5yxo{1o{ecg9o{0{eh7do{1o{enwho{0{esfto{1o{ez4xo{0{f3o9o{1o{faddo{0{ff4ho{1o{fllto{0{fqcxo{1o{fwu9o{0{g1ldo{1o{g8aho{0",
      },
    ],
  },
  {
    countryCode: "FR",
    locales: [
      {
        ianaTimezone: "Europe/Paris",
        name: "(GMT+01:00) Paris",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "GA",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "GB",
    locales: [
      {
        ianaTimezone: "Europe/London",
        name: "(GMT+00:00) London",
        data: "0{e5yxo{1o{ecg9o{0{eh7do{1o{enwho{0{esfto{1o{ez4xo{0{f3o9o{1o{faddo{0{ff4ho{1o{fllto{0{fqcxo{1o{fwu9o{0{g1ldo{1o{g8aho{0",
      },
    ],
  },
  {
    countryCode: "GD",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "GE",
    locales: [
      {
        ianaTimezone: "Asia/Tbilisi",
        name: "(GMT+04:00) Tbilisi",
        data: "6o",
      },
    ],
  },
  {
    countryCode: "GF",
    locales: [
      {
        ianaTimezone: "America/Cayenne",
        name: "(GMT-03:00) Cayenne",
        data: "-50",
      },
    ],
  },
  {
    countryCode: "GG",
    locales: [
      {
        ianaTimezone: "Europe/London",
        name: "(GMT+00:00) London",
        data: "0{e5yxo{1o{ecg9o{0{eh7do{1o{enwho{0{esfto{1o{ez4xo{0{f3o9o{1o{faddo{0{ff4ho{1o{fllto{0{fqcxo{1o{fwu9o{0{g1ldo{1o{g8aho{0",
      },
    ],
  },
  {
    countryCode: "GH",
    locales: [
      {
        ianaTimezone: "Africa/Accra",
        name: "(GMT+00:00) Accra",
        data: "0",
      },
    ],
  },
  {
    countryCode: "GI",
    locales: [
      {
        ianaTimezone: "Europe/Gibraltar",
        name: "(GMT+01:00) Gibraltar",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "GL",
    locales: [
      {
        ianaTimezone: "America/Thule",
        name: "(GMT-04:00) Thule",
        data: "-6o{e5bu0{-50{eco8c{-6o{egs20{-50{eo4gc{-6o{es0i0{-50{ezcwc{-6o{f38y0{-50{falcc{-6o{fehe0{-50{fltsc{-6o{fppu0{-50{fx28c{-6o{g1620{-50{g8igc{-6o",
      },
      {
        ianaTimezone: "America/Godthab",
        name: "(GMT-03:00) Godthab",
        data: "-50{e5yxo{-3c{ecg9o{-50{eh7do{-3c{enwho{-50{esfto{-3c{ez4xo{-50{f3o9o{-3c{faddo{-50{ff4ho{-3c{fllto{-50{fqcxo{-3c{fwu9o{-50{g1ldo{-3c{g8aho{-50",
      },
      {
        ianaTimezone: "America/Scoresbysund",
        name: "(GMT-01:00) Scoresbysund",
        data: "-1o{e5yxo{0{ecg9o{-1o{eh7do{0{enwho{-1o{esfto{0{ez4xo{-1o{f3o9o{0{faddo{-1o{ff4ho{0{fllto{-1o{fqcxo{0{fwu9o{-1o{g1ldo{0{g8aho{-1o",
      },
      {
        ianaTimezone: "America/Danmarkshavn",
        name: "(GMT+00:00) Danmarkshavn",
        data: "0",
      },
    ],
  },
  {
    countryCode: "GM",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "GN",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "GP",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "GQ",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "GR",
    locales: [
      {
        ianaTimezone: "Europe/Athens",
        name: "(GMT+02:00) Athens",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "GS",
    locales: [
      {
        ianaTimezone: "Atlantic/South_Georgia",
        name: "(GMT-02:00) South Georgia",
        data: "-3c",
      },
    ],
  },
  {
    countryCode: "GT",
    locales: [
      {
        ianaTimezone: "America/Guatemala",
        name: "(GMT-06:00) Guatemala",
        data: "-a0",
      },
    ],
  },
  {
    countryCode: "GU",
    locales: [
      {
        ianaTimezone: "Pacific/Guam",
        name: "(GMT+10:00) Guam",
        data: "go",
      },
    ],
  },
  {
    countryCode: "GW",
    locales: [
      {
        ianaTimezone: "Africa/Bissau",
        name: "(GMT+00:00) Bissau",
        data: "0",
      },
    ],
  },
  {
    countryCode: "GY",
    locales: [
      {
        ianaTimezone: "America/Guyana",
        name: "(GMT-04:00) Guyana",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "HK",
    locales: [
      {
        ianaTimezone: "Asia/Hong_Kong",
        name: "(GMT+08:00) Hong Kong",
        data: "dc",
      },
    ],
  },
  {
    countryCode: "HM",
    locales: [
      {
        ianaTimezone: "Indian/Kerguelen",
        name: "(GMT+05:00) Kerguelen",
        data: "8c",
      },
    ],
  },
  {
    countryCode: "HN",
    locales: [
      {
        ianaTimezone: "America/Tegucigalpa",
        name: "(GMT-06:00) Central Time - Tegucigalpa",
        data: "-a0",
      },
    ],
  },
  {
    countryCode: "HR",
    locales: [
      {
        ianaTimezone: "Europe/Belgrade",
        name: "(GMT+01:00) Central European Time - Belgrade",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "HT",
    locales: [
      {
        ianaTimezone: "America/Port-au-Prince",
        name: "(GMT-05:00) Port-au-Prince",
        data: "-8c{e5bvo{-6o{ecoa0{-8c",
      },
    ],
  },
  {
    countryCode: "HU",
    locales: [
      {
        ianaTimezone: "Europe/Budapest",
        name: "(GMT+01:00) Budapest",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "ID",
    locales: [
      {
        ianaTimezone: "Asia/Jakarta",
        name: "(GMT+07:00) Jakarta",
        data: "bo",
      },
      {
        ianaTimezone: "Asia/Makassar",
        name: "(GMT+08:00) Makassar",
        data: "dc",
      },
      {
        ianaTimezone: "Asia/Jayapura",
        name: "(GMT+09:00) Jayapura",
        data: "f0",
      },
    ],
  },
  {
    countryCode: "IE",
    locales: [
      {
        ianaTimezone: "Europe/Dublin",
        name: "(GMT+00:00) Dublin",
        data: "0{e5yxo{1o{ecg9o{0{eh7do{1o{enwho{0{esfto{1o{ez4xo{0{f3o9o{1o{faddo{0{ff4ho{1o{fllto{0{fqcxo{1o{fwu9o{0{g1ldo{1o{g8aho{0",
      },
    ],
  },
  {
    countryCode: "IL",
    locales: [
      {
        ianaTimezone: "Asia/Jerusalem",
        name: "(GMT+02:00) Jerusalem",
        data: "3c{e5wo0{50{ecg6c{3c{eh540{50{enwec{3c{esdk0{50{ez4uc{3c{f3m00{50{fadac{3c{ff280{50{fllqc{3c{fqao0{50{fwu6c{3c{g1j40{50{g8aec{3c",
      },
    ],
  },
  {
    countryCode: "IM",
    locales: [
      {
        ianaTimezone: "Europe/London",
        name: "(GMT+00:00) London",
        data: "0{e5yxo{1o{ecg9o{0{eh7do{1o{enwho{0{esfto{1o{ez4xo{0{f3o9o{1o{faddo{0{ff4ho{1o{fllto{0{fqcxo{1o{fwu9o{0{g1ldo{1o{g8aho{0",
      },
    ],
  },
  {
    countryCode: "IN",
    locales: [
      {
        ianaTimezone: "Asia/Calcutta",
        name: "(GMT+05:30) India Standard Time",
        data: "96",
      },
    ],
  },
  {
    countryCode: "IO",
    locales: [
      {
        ianaTimezone: "Indian/Chagos",
        name: "(GMT+06:00) Chagos",
        data: "a0",
      },
    ],
  },
  {
    countryCode: "IQ",
    locales: [
      {
        ianaTimezone: "Asia/Baghdad",
        name: "(GMT+03:00) Baghdad",
        data: "50",
      },
    ],
  },
  {
    countryCode: "IR",
    locales: [
      {
        ianaTimezone: "Asia/Tehran",
        name: "(GMT+03:30) Tehran",
        data: "5u{e5qy6{7i{ebfci{5u{eh0i6{7i{emowi{5u{esb66{7i{exzki{5u{f3kq6{7i{f994i{5u{feua6{7i{fkioi{5u{fq3u6{7i{fvs8i{5u{g1ei6{7i{g72wi{5u",
      },
    ],
  },
  {
    countryCode: "IS",
    locales: [
      {
        ianaTimezone: "Atlantic/Reykjavik",
        name: "(GMT+00:00) Reykjavik",
        data: "0",
      },
    ],
  },
  {
    countryCode: "IT",
    locales: [
      {
        ianaTimezone: "Europe/Rome",
        name: "(GMT+01:00) Rome",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "JE",
    locales: [
      {
        ianaTimezone: "Europe/London",
        name: "(GMT+00:00) London",
        data: "0{e5yxo{1o{ecg9o{0{eh7do{1o{enwho{0{esfto{1o{ez4xo{0{f3o9o{1o{faddo{0{ff4ho{1o{fllto{0{fqcxo{1o{fwu9o{0{g1ldo{1o{g8aho{0",
      },
    ],
  },
  {
    countryCode: "JM",
    locales: [
      {
        ianaTimezone: "America/Jamaica",
        name: "(GMT-05:00) Jamaica",
        data: "-8c",
      },
    ],
  },
  {
    countryCode: "JO",
    locales: [
      {
        ianaTimezone: "Asia/Amman",
        name: "(GMT+02:00) Amman",
        data: "3c{e5wko{50{ecloo{3c{ehcso{50{enu4o{3c{esl8o{50{ez2ko{3c{f3too{50{fab0o{3c{ff24o{50{fljgo{3c{fqako{50{fwzoo{3c{g1j0o{50{g884o{3c",
      },
    ],
  },
  {
    countryCode: "JP",
    locales: [
      {
        ianaTimezone: "Asia/Tokyo",
        name: "(GMT+09:00) Tokyo",
        data: "f0",
      },
    ],
  },
  {
    countryCode: "KE",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "KG",
    locales: [
      {
        ianaTimezone: "Asia/Bishkek",
        name: "(GMT+06:00) Bishkek",
        data: "a0",
      },
    ],
  },
  {
    countryCode: "KH",
    locales: [
      {
        ianaTimezone: "Asia/Bangkok",
        name: "(GMT+07:00) Bangkok",
        data: "bo",
      },
    ],
  },
  {
    countryCode: "KI",
    locales: [
      {
        ianaTimezone: "Pacific/Tarawa",
        name: "(GMT+12:00) Tarawa",
        data: "k0",
      },
      {
        ianaTimezone: "Pacific/Kiritimati",
        name: "(GMT+14:00) Kiritimati",
        data: "nc",
      },
    ],
  },
  {
    countryCode: "KM",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "KN",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "KP",
    locales: [
      {
        ianaTimezone: "Asia/Pyongyang",
        name: "(GMT+08:30) Pyongyang",
        data: "f0{ea8x0{e6",
      },
    ],
  },
  {
    countryCode: "KR",
    locales: [
      {
        ianaTimezone: "Asia/Seoul",
        name: "(GMT+09:00) Seoul",
        data: "f0",
      },
    ],
  },
  {
    countryCode: "KW",
    locales: [
      {
        ianaTimezone: "Asia/Riyadh",
        name: "(GMT+03:00) Riyadh",
        data: "50",
      },
    ],
  },
  {
    countryCode: "KY",
    locales: [
      {
        ianaTimezone: "America/Panama",
        name: "(GMT-05:00) Panama",
        data: "-8c",
      },
    ],
  },
  {
    countryCode: "KZ",
    locales: [
      {
        ianaTimezone: "Asia/Aqtau",
        name: "(GMT+05:00) Aqtau",
        data: "8c",
      },
      {
        ianaTimezone: "Asia/Aqtobe",
        name: "(GMT+05:00) Aqtobe",
        data: "8c",
      },
      {
        ianaTimezone: "Asia/Almaty",
        name: "(GMT+06:00) Almaty",
        data: "a0",
      },
    ],
  },
  {
    countryCode: "LA",
    locales: [
      {
        ianaTimezone: "Asia/Bangkok",
        name: "(GMT+07:00) Bangkok",
        data: "bo",
      },
    ],
  },
  {
    countryCode: "LB",
    locales: [
      {
        ianaTimezone: "Asia/Beirut",
        name: "(GMT+02:00) Beirut",
        data: "3c{e5yso{50{ecg30{3c{eh78o{50{enwb0{3c{esfoo{50{ez4r0{3c{f3o4o{50{fad70{3c{ff4co{50{flln0{3c{fqcso{50{fwu30{3c{g1l8o{50{g8ab0{3c",
      },
    ],
  },
  {
    countryCode: "LC",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "LI",
    locales: [
      {
        ianaTimezone: "Europe/Zurich",
        name: "(GMT+01:00) Zurich",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "LK",
    locales: [
      {
        ianaTimezone: "Asia/Colombo",
        name: "(GMT+05:30) Colombo",
        data: "96",
      },
    ],
  },
  {
    countryCode: "LR",
    locales: [
      {
        ianaTimezone: "Africa/Monrovia",
        name: "(GMT+00:00) Monrovia",
        data: "0",
      },
    ],
  },
  {
    countryCode: "LS",
    locales: [
      {
        ianaTimezone: "Africa/Johannesburg",
        name: "(GMT+02:00) Johannesburg",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "LT",
    locales: [
      {
        ianaTimezone: "Europe/Vilnius",
        name: "(GMT+02:00) Vilnius",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "LU",
    locales: [
      {
        ianaTimezone: "Europe/Luxembourg",
        name: "(GMT+01:00) Luxembourg",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "LV",
    locales: [
      {
        ianaTimezone: "Europe/Riga",
        name: "(GMT+02:00) Riga",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "LY",
    locales: [
      {
        ianaTimezone: "Africa/Tripoli",
        name: "(GMT+02:00) Tripoli",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "MA",
    locales: [
      {
        ianaTimezone: "Africa/Casablanca",
        name: "(GMT+00:00) Casablanca",
        data: "0{e5yzc{1o{e8cjc{0{e9ffc{1o{ecgbc{0{eh7fc{1o{ejd7c{0{ekg3c{1o{enwjc{0{esfvc{1o{eu63c{0{evgrc{1o{ez4zc{0{f3obc{1o{f56rc{0{f69nc{1o{fadfc{0{ff4jc{1o{fg7fc{0{fhabc{1o{fllvc{0{fqczc{1o{fr0bc{0{fs37c{1o{fwubc{0{g1lfc{1o{g20zc{0{g33vc{1o{g8ajc{0",
      },
    ],
  },
  {
    countryCode: "MC",
    locales: [
      {
        ianaTimezone: "Europe/Monaco",
        name: "(GMT+01:00) Monaco",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "MD",
    locales: [
      {
        ianaTimezone: "Europe/Chisinau",
        name: "(GMT+02:00) Chisinau",
        data: "3c{e5yw0{50{ecg80{3c{eh7c0{50{enwg0{3c{esfs0{50{ez4w0{3c{f3o80{50{fadc0{3c{ff4g0{50{flls0{3c{fqcw0{50{fwu80{3c{g1lc0{50{g8ag0{3c",
      },
    ],
  },
  {
    countryCode: "ME",
    locales: [
      {
        ianaTimezone: "Europe/Belgrade",
        name: "(GMT+01:00) Central European Time - Belgrade",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "MF",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "MG",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "MH",
    locales: [
      {
        ianaTimezone: "Pacific/Kwajalein",
        name: "(GMT+12:00) Kwajalein",
        data: "k0",
      },
      {
        ianaTimezone: "Pacific/Majuro",
        name: "(GMT+12:00) Majuro",
        data: "k0",
      },
    ],
  },
  {
    countryCode: "MK",
    locales: [
      {
        ianaTimezone: "Europe/Belgrade",
        name: "(GMT+01:00) Central European Time - Belgrade",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "ML",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "MN",
    locales: [
      {
        ianaTimezone: "Asia/Hovd",
        name: "(GMT+07:00) Hovd",
        data: "bo{e5xjo{dc{ebjmo{bo{eh5zo{dc{ems2o{bo{esefo{dc{ey8ao{bo{f3uno{dc{f9gqo{bo{ff33o{dc{fkp6o{bo{fqbjo{dc{fvxmo{bo{g1jzo{dc{g762o{bo",
      },
      {
        ianaTimezone: "Asia/Choibalsan",
        name: "(GMT+08:00) Choibalsan",
        data: "dc{e5xi0{f0{ebjl0{dc{eh5y0{f0{ems10{dc{esee0{f0{ey890{dc{f3um0{f0{f9gp0{dc{ff320{f0{fkp50{dc{fqbi0{f0{fvxl0{dc{g1jy0{f0{g7610{dc",
      },
      {
        ianaTimezone: "Asia/Ulaanbaatar",
        name: "(GMT+08:00) Ulaanbaatar",
        data: "dc{e5xi0{f0{ebjl0{dc{eh5y0{f0{ems10{dc{esee0{f0{ey890{dc{f3um0{f0{f9gp0{dc{ff320{f0{fkp50{dc{fqbi0{f0{fvxl0{dc{g1jy0{f0{g7610{dc",
      },
    ],
  },
  {
    countryCode: "MO",
    locales: [
      {
        ianaTimezone: "Asia/Macau",
        name: "(GMT+08:00) Macau",
        data: "dc",
      },
    ],
  },
  {
    countryCode: "MP",
    locales: [
      {
        ianaTimezone: "Pacific/Guam",
        name: "(GMT+10:00) Guam",
        data: "go",
      },
    ],
  },
  {
    countryCode: "MQ",
    locales: [
      {
        ianaTimezone: "America/Martinique",
        name: "(GMT-04:00) Martinique",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "MR",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "MS",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "MT",
    locales: [
      {
        ianaTimezone: "Europe/Malta",
        name: "(GMT+01:00) Malta",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "MU",
    locales: [
      {
        ianaTimezone: "Indian/Mauritius",
        name: "(GMT+04:00) Mauritius",
        data: "6o",
      },
    ],
  },
  {
    countryCode: "MV",
    locales: [
      {
        ianaTimezone: "Indian/Maldives",
        name: "(GMT+05:00) Maldives",
        data: "8c",
      },
    ],
  },
  {
    countryCode: "MW",
    locales: [
      {
        ianaTimezone: "Africa/Maputo",
        name: "(GMT+02:00) Maputo",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "MX",
    locales: [
      {
        ianaTimezone: "America/Tijuana",
        name: "(GMT-08:00) Pacific Time - Tijuana",
        data: "-dc{e5c0o{-bo{ecof0{-dc{egs8o{-bo{eo4n0{-dc{es0oo{-bo{ezd30{-dc{f394o{-bo{falj0{-dc{fehko{-bo{fltz0{-dc{fpq0o{-bo{fx2f0{-dc{g168o{-bo{g8in0{-dc",
      },
      {
        ianaTimezone: "America/Hermosillo",
        name: "(GMT-07:00) Mountain Time - Hermosillo",
        data: "-bo",
      },
      {
        ianaTimezone: "America/Mazatlan",
        name: "(GMT-07:00) Mountain Time - Chihuahua, Mazatlan",
        data: "-bo{e6730{-a0{ecglc{-bo{ehfj0{-a0{enwtc{-bo{esnz0{-a0{ez59c{-bo{f3wf0{-a0{fadpc{-bo{ffcn0{-a0{flm5c{-bo{fql30{-a0{fwulc{-bo{g1tj0{-a0{g8atc{-bo",
      },
      {
        ianaTimezone: "America/Mexico_City",
        name: "(GMT-06:00) Central Time - Mexico City",
        data: "-a0{e671c{-8c{ecgjo{-a0{ehfhc{-8c{enwro{-a0{esnxc{-8c{ez57o{-a0{f3wdc{-8c{fadno{-a0{ffclc{-8c{flm3o{-a0{fql1c{-8c{fwujo{-a0{g1thc{-8c{g8aro{-a0",
      },
      {
        ianaTimezone: "America/Cancun",
        name: "(GMT-05:00) America Cancun",
        data: "-a0{e491c{-8c",
      },
    ],
  },
  {
    countryCode: "MY",
    locales: [
      {
        ianaTimezone: "Asia/Kuala_Lumpur",
        name: "(GMT+08:00) Kuala Lumpur",
        data: "dc",
      },
    ],
  },
  {
    countryCode: "MZ",
    locales: [
      {
        ianaTimezone: "Africa/Maputo",
        name: "(GMT+02:00) Maputo",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "NA",
    locales: [
      {
        ianaTimezone: "Africa/Windhoek",
        name: "(GMT+02:00) Windhoek",
        data: "3c{e66o0{1o{eaxto{3c{ehf40{1o{em69o{3c{esnk0{1o{exepo{3c{f3w00{1o{f8n5o{3c{ffc80{1o{fjvlo{3c{fqko0{1o{fvbto{3c{g1t40{1o{g6k9o{3c",
      },
    ],
  },
  {
    countryCode: "NC",
    locales: [
      {
        ianaTimezone: "Pacific/Noumea",
        name: "(GMT+11:00) Noumea",
        data: "ic",
      },
    ],
  },
  {
    countryCode: "NE",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "NF",
    locales: [
      {
        ianaTimezone: "Pacific/Norfolk",
        name: "(GMT+11:00) Norfolk",
        data: "j6{ebsg6{ic",
      },
    ],
  },
  {
    countryCode: "NG",
    locales: [
      {
        ianaTimezone: "Africa/Lagos",
        name: "(GMT+01:00) Lagos",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "NI",
    locales: [
      {
        ianaTimezone: "America/Managua",
        name: "(GMT-06:00) Managua",
        data: "-a0",
      },
    ],
  },
  {
    countryCode: "NL",
    locales: [
      {
        ianaTimezone: "Europe/Amsterdam",
        name: "(GMT+01:00) Amsterdam",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "NO",
    locales: [
      {
        ianaTimezone: "Europe/Oslo",
        name: "(GMT+01:00) Oslo",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "NP",
    locales: [
      {
        ianaTimezone: "Asia/Katmandu",
        name: "(GMT+05:45) Katmandu",
        data: "9l",
      },
    ],
  },
  {
    countryCode: "NR",
    locales: [
      {
        ianaTimezone: "Pacific/Nauru",
        name: "(GMT+12:00) Nauru",
        data: "k0",
      },
    ],
  },
  {
    countryCode: "NU",
    locales: [
      {
        ianaTimezone: "Pacific/Niue",
        name: "(GMT-11:00) Niue",
        data: "-ic",
      },
    ],
  },
  {
    countryCode: "NZ",
    locales: [
      {
        ianaTimezone: "Pacific/Auckland",
        name: "(GMT+13:00) Auckland",
        data: "lo{e667c{k0{ebknc{lo{ehenc{k0{emt3c{lo{esn3c{k0{ey1jc{lo{f3vjc{k0{f9hrc{lo{ffbrc{k0{fkq7c{lo{fqk7c{k0{fvync{lo{g1snc{k0{g773c{lo",
      },
    ],
  },
  {
    countryCode: "OM",
    locales: [
      {
        ianaTimezone: "Asia/Dubai",
        name: "(GMT+04:00) Dubai",
        data: "6o",
      },
    ],
  },
  {
    countryCode: "PA",
    locales: [
      {
        ianaTimezone: "America/Panama",
        name: "(GMT-05:00) Panama",
        data: "-8c",
      },
    ],
  },
  {
    countryCode: "PE",
    locales: [
      {
        ianaTimezone: "America/Lima",
        name: "(GMT-05:00) Lima",
        data: "-8c",
      },
    ],
  },
  {
    countryCode: "PF",
    locales: [
      {
        ianaTimezone: "Pacific/Tahiti",
        name: "(GMT-10:00) Tahiti",
        data: "-go",
      },
      {
        ianaTimezone: "Pacific/Marquesas",
        name: "(GMT-09:30) Marquesas",
        data: "-fu",
      },
      {
        ianaTimezone: "Pacific/Gambier",
        name: "(GMT-09:00) Gambier",
        data: "-f0",
      },
    ],
  },
  {
    countryCode: "PG",
    locales: [
      {
        ianaTimezone: "Pacific/Port_Moresby",
        name: "(GMT+10:00) Port Moresby",
        data: "go",
      },
    ],
  },
  {
    countryCode: "PH",
    locales: [
      {
        ianaTimezone: "Asia/Manila",
        name: "(GMT+08:00) Manila",
        data: "dc",
      },
    ],
  },
  {
    countryCode: "PK",
    locales: [
      {
        ianaTimezone: "Asia/Karachi",
        name: "(GMT+05:00) Karachi",
        data: "8c",
      },
    ],
  },
  {
    countryCode: "PL",
    locales: [
      {
        ianaTimezone: "Europe/Warsaw",
        name: "(GMT+01:00) Warsaw",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "PM",
    locales: [
      {
        ianaTimezone: "America/Miquelon",
        name: "(GMT-03:00) Miquelon",
        data: "-50{e5bsc{-3c{eco6o{-50{egs0c{-3c{eo4eo{-50{es0gc{-3c{ezcuo{-50{f38wc{-3c{falao{-50{fehcc{-3c{fltqo{-50{fppsc{-3c{fx26o{-50{g160c{-3c{g8ieo{-50",
      },
    ],
  },
  {
    countryCode: "PN",
    locales: [
      {
        ianaTimezone: "Pacific/Pitcairn",
        name: "(GMT-08:00) Pitcairn",
        data: "-dc",
      },
    ],
  },
  {
    countryCode: "PR",
    locales: [
      {
        ianaTimezone: "America/Puerto_Rico",
        name: "(GMT-04:00) Puerto Rico",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "PS",
    locales: [
      {
        ianaTimezone: "Asia/Gaza",
        name: "(GMT+02:00) Gaza",
        data: "3c{e5xoo{50{ecdv0{3c{eh66c{50{env8o{3c{esemc{50{ez3oo{3c{f3uuc{50{fac4o{3c{ff3ac{50{flkko{3c{fqbqc{50{fx0so{3c{g1k6c{50{g898o{3c",
      },
    ],
  },
  {
    countryCode: "PT",
    locales: [
      {
        ianaTimezone: "Atlantic/Azores",
        name: "(GMT-01:00) Azores",
        data: "-1o{e5yxo{0{ecg9o{-1o{eh7do{0{enwho{-1o{esfto{0{ez4xo{-1o{f3o9o{0{faddo{-1o{ff4ho{0{fllto{-1o{fqcxo{0{fwu9o{-1o{g1ldo{0{g8aho{-1o",
      },
      {
        ianaTimezone: "Europe/Lisbon",
        name: "(GMT+00:00) Lisbon",
        data: "0{e5yxo{1o{ecg9o{0{eh7do{1o{enwho{0{esfto{1o{ez4xo{0{f3o9o{1o{faddo{0{ff4ho{1o{fllto{0{fqcxo{1o{fwu9o{0{g1ldo{1o{g8aho{0",
      },
    ],
  },
  {
    countryCode: "PW",
    locales: [
      {
        ianaTimezone: "Pacific/Palau",
        name: "(GMT+09:00) Palau",
        data: "f0",
      },
    ],
  },
  {
    countryCode: "PY",
    locales: [
      {
        ianaTimezone: "America/Asuncion",
        name: "(GMT-03:00) Asuncion",
        data: "-50{e5r90{-6o{ebt2o{-50{eh7h0{-6o{en1io{-50{esfx0{-6o{ey9yo{-50{f3od0{-6o{f9q6o{-50{fewt0{-6o{fkymo{-50{fq590{-6o{fw72o{-50{g1lh0{-6o{g7fio{-50",
      },
    ],
  },
  {
    countryCode: "QA",
    locales: [
      {
        ianaTimezone: "Asia/Qatar",
        name: "(GMT+03:00) Qatar",
        data: "50",
      },
    ],
  },
  {
    countryCode: "RE",
    locales: [
      {
        ianaTimezone: "Indian/Reunion",
        name: "(GMT+04:00) Reunion",
        data: "6o",
      },
    ],
  },
  {
    countryCode: "RO",
    locales: [
      {
        ianaTimezone: "Europe/Bucharest",
        name: "(GMT+02:00) Bucharest",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "RS",
    locales: [
      {
        ianaTimezone: "Europe/Belgrade",
        name: "(GMT+01:00) Central European Time - Belgrade",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "RU",
    locales: [
      {
        ianaTimezone: "Europe/Kaliningrad",
        name: "(GMT+02:00) Moscow-01 - Kaliningrad",
        data: "3c",
      },
      {
        ianaTimezone: "Europe/Moscow",
        name: "(GMT+03:00) Moscow+00 - Moscow",
        data: "50",
      },
      {
        ianaTimezone: "Europe/Samara",
        name: "(GMT+04:00) Moscow+01 - Samara",
        data: "6o",
      },
      {
        ianaTimezone: "Asia/Yekaterinburg",
        name: "(GMT+05:00) Moscow+02 - Yekaterinburg",
        data: "8c",
      },
      {
        ianaTimezone: "Asia/Omsk",
        name: "(GMT+06:00) Moscow+03 - Omsk",
        data: "a0",
      },
      {
        ianaTimezone: "Asia/Krasnoyarsk",
        name: "(GMT+07:00) Moscow+04 - Krasnoyarsk",
        data: "bo",
      },
      {
        ianaTimezone: "Asia/Irkutsk",
        name: "(GMT+08:00) Moscow+05 - Irkutsk",
        data: "dc",
      },
      {
        ianaTimezone: "Asia/Yakutsk",
        name: "(GMT+09:00) Moscow+06 - Yakutsk",
        data: "f0",
      },
      {
        ianaTimezone: "Asia/Vladivostok",
        name: "(GMT+10:00) Moscow+07 - Vladivostok",
        data: "go",
      },
      {
        ianaTimezone: "Asia/Magadan",
        name: "(GMT+11:00) Moscow+08 - Magadan",
        data: "go{ei22o{ic",
      },
      {
        ianaTimezone: "Asia/Kamchatka",
        name: "(GMT+12:00) Moscow+09 - Petropavlovsk-Kamchatskiy",
        data: "k0",
      },
    ],
  },
  {
    countryCode: "RW",
    locales: [
      {
        ianaTimezone: "Africa/Maputo",
        name: "(GMT+02:00) Maputo",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "SA",
    locales: [
      {
        ianaTimezone: "Asia/Riyadh",
        name: "(GMT+03:00) Riyadh",
        data: "50",
      },
    ],
  },
  {
    countryCode: "SB",
    locales: [
      {
        ianaTimezone: "Pacific/Guadalcanal",
        name: "(GMT+11:00) Guadalcanal",
        data: "ic",
      },
    ],
  },
  {
    countryCode: "SC",
    locales: [
      {
        ianaTimezone: "Indian/Mahe",
        name: "(GMT+04:00) Mahe",
        data: "6o",
      },
    ],
  },
  {
    countryCode: "SD",
    locales: [
      {
        ianaTimezone: "Africa/Khartoum",
        name: "(GMT+03:00) Khartoum",
        data: "50",
      },
    ],
  },
  {
    countryCode: "SE",
    locales: [
      {
        ianaTimezone: "Europe/Stockholm",
        name: "(GMT+01:00) Stockholm",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "SG",
    locales: [
      {
        ianaTimezone: "Asia/Singapore",
        name: "(GMT+08:00) Singapore",
        data: "dc",
      },
    ],
  },
  {
    countryCode: "SH",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "SI",
    locales: [
      {
        ianaTimezone: "Europe/Belgrade",
        name: "(GMT+01:00) Central European Time - Belgrade",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "SJ",
    locales: [
      {
        ianaTimezone: "Europe/Oslo",
        name: "(GMT+01:00) Oslo",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "SK",
    locales: [
      {
        ianaTimezone: "Europe/Prague",
        name: "(GMT+01:00) Central European Time - Prague",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "SL",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "SM",
    locales: [
      {
        ianaTimezone: "Europe/Rome",
        name: "(GMT+01:00) Rome",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "SN",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "SO",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "SR",
    locales: [
      {
        ianaTimezone: "America/Paramaribo",
        name: "(GMT-03:00) Paramaribo",
        data: "-50",
      },
    ],
  },
  {
    countryCode: "SS",
    locales: [
      {
        ianaTimezone: "Africa/Khartoum",
        name: "(GMT+03:00) Khartoum",
        data: "50",
      },
    ],
  },
  {
    countryCode: "ST",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "SV",
    locales: [
      {
        ianaTimezone: "America/El_Salvador",
        name: "(GMT-06:00) El Salvador",
        data: "-a0",
      },
    ],
  },
  {
    countryCode: "SX",
    locales: [
      {
        ianaTimezone: "America/Curacao",
        name: "(GMT-04:00) Curacao",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "SY",
    locales: [
      {
        ianaTimezone: "Asia/Damascus",
        name: "(GMT+02:00) Damascus",
        data: "3c{e5wko{50{ecln0{3c{eh50o{50{enu30{3c{esl8o{50{ez2j0{3c{f3too{50{faaz0{3c{ff24o{50{fljf0{3c{fqako{50{fwzn0{3c{g1j0o{50{g8830{3c",
      },
    ],
  },
  {
    countryCode: "SZ",
    locales: [
      {
        ianaTimezone: "Africa/Johannesburg",
        name: "(GMT+02:00) Johannesburg",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "TC",
    locales: [
      {
        ianaTimezone: "America/Grand_Turk",
        name: "(GMT-04:00) Grand Turk",
        data: "-8c{e5bvo{-6o{ecoa0{-6o",
      },
    ],
  },
  {
    countryCode: "TD",
    locales: [
      {
        ianaTimezone: "Africa/Ndjamena",
        name: "(GMT+01:00) Ndjamena",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "TF",
    locales: [
      {
        ianaTimezone: "Indian/Kerguelen",
        name: "(GMT+05:00) Kerguelen",
        data: "8c",
      },
    ],
  },
  {
    countryCode: "TG",
    locales: [
      {
        ianaTimezone: "Africa/Abidjan",
        name: "(GMT+00:00) Abidjan",
        data: "0",
      },
    ],
  },
  {
    countryCode: "TH",
    locales: [
      {
        ianaTimezone: "Asia/Bangkok",
        name: "(GMT+07:00) Bangkok",
        data: "bo",
      },
    ],
  },
  {
    countryCode: "TJ",
    locales: [
      {
        ianaTimezone: "Asia/Dushanbe",
        name: "(GMT+05:00) Dushanbe",
        data: "8c",
      },
    ],
  },
  {
    countryCode: "TK",
    locales: [
      {
        ianaTimezone: "Pacific/Fakaofo",
        name: "(GMT+13:00) Fakaofo",
        data: "lo",
      },
    ],
  },
  {
    countryCode: "TL",
    locales: [
      {
        ianaTimezone: "Asia/Dili",
        name: "(GMT+09:00) Dili",
        data: "f0",
      },
    ],
  },
  {
    countryCode: "TM",
    locales: [
      {
        ianaTimezone: "Asia/Ashgabat",
        name: "(GMT+05:00) Ashgabat",
        data: "8c",
      },
    ],
  },
  {
    countryCode: "TN",
    locales: [
      {
        ianaTimezone: "Africa/Tunis",
        name: "(GMT+01:00) Tunis",
        data: "1o",
      },
    ],
  },
  {
    countryCode: "TO",
    locales: [
      {
        ianaTimezone: "Pacific/Tongatapu",
        name: "(GMT+14:00) Tongatapu",
        data: "lo{eo3po{nc{eq9ho{lo{ezc5o{nc{f1ppo{lo{faklo{nc{fcy5o{lo{flt1o{nc{fo6lo{lo{fx1ho{nc{fzf1o{lo{g8hpo{nc",
      },
    ],
  },
  {
    countryCode: "TR",
    locales: [
      {
        ianaTimezone: "Europe/Istanbul",
        name: "(GMT+03:00) Istanbul",
        data: "3c{e5yxo{50{ecvto{3c{eh7do{50{em9f0{50",
      },
    ],
  },
  {
    countryCode: "TT",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "TV",
    locales: [
      {
        ianaTimezone: "Pacific/Funafuti",
        name: "(GMT+12:00) Funafuti",
        data: "k0",
      },
    ],
  },
  {
    countryCode: "TW",
    locales: [
      {
        ianaTimezone: "Asia/Taipei",
        name: "(GMT+08:00) Taipei",
        data: "dc",
      },
    ],
  },
  {
    countryCode: "TZ",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "UA",
    locales: [
      {
        ianaTimezone: "Europe/Kiev",
        name: "(GMT+02:00) Kiev",
        data: "3c{e5yxo{50{ecg9o{3c{eh7do{50{enwho{3c{esfto{50{ez4xo{3c{f3o9o{50{faddo{3c{ff4ho{50{fllto{3c{fqcxo{50{fwu9o{3c{g1ldo{50{g8aho{3c",
      },
    ],
  },
  {
    countryCode: "UG",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "UM",
    locales: [
      {
        ianaTimezone: "Pacific/Pago_Pago",
        name: "(GMT-11:00) Pago Pago",
        data: "-ic",
      },
      {
        ianaTimezone: "Pacific/Honolulu",
        name: "(GMT-10:00) Hawaii Time",
        data: "-go",
      },
      {
        ianaTimezone: "Pacific/Wake",
        name: "(GMT+12:00) Wake",
        data: "k0",
      },
      {
        ianaTimezone: "Pacific/Enderbury",
        name: "(GMT+13:00) Enderbury",
        data: "lo",
      },
    ],
  },
  {
    countryCode: "US",
    locales: [
      {
        ianaTimezone: "Pacific/Honolulu",
        name: "(GMT-10:00) Hawaii Time",
        data: "-go",
      },
      {
        ianaTimezone: "America/Anchorage",
        name: "(GMT-09:00) Alaska Time",
        data: "-f0{e5c2c{-dc{ecogo{-f0{egsac{-dc{eo4oo{-f0{es0qc{-dc{ezd4o{-f0{f396c{-dc{falko{-f0{fehmc{-dc{flu0o{-f0{fpq2c{-dc{fx2go{-f0{g16ac{-dc{g8ioo{-f0",
      },
      {
        ianaTimezone: "America/Los_Angeles",
        name: "(GMT-08:00) Pacific Time",
        data: "-dc{e5c0o{-bo{ecof0{-dc{egs8o{-bo{eo4n0{-dc{es0oo{-bo{ezd30{-dc{f394o{-bo{falj0{-dc{fehko{-bo{fltz0{-dc{fpq0o{-bo{fx2f0{-dc{g168o{-bo{g8in0{-dc",
      },
      {
        ianaTimezone: "America/Denver",
        name: "(GMT-07:00) Mountain Time",
        data: "-bo{e5bz0{-a0{ecodc{-bo{egs70{-a0{eo4lc{-bo{es0n0{-a0{ezd1c{-bo{f3930{-a0{falhc{-bo{fehj0{-a0{fltxc{-bo{fppz0{-a0{fx2dc{-bo{g1670{-a0{g8ilc{-bo",
      },
      {
        ianaTimezone: "America/Phoenix",
        name: "(GMT-07:00) Mountain Time - Arizona",
        data: "-bo",
      },
      {
        ianaTimezone: "America/Chicago",
        name: "(GMT-06:00) Central Time",
        data: "-a0{e5bxc{-8c{ecobo{-a0{egs5c{-8c{eo4jo{-a0{es0lc{-8c{ezczo{-a0{f391c{-8c{falfo{-a0{fehhc{-8c{fltvo{-a0{fppxc{-8c{fx2bo{-a0{g165c{-8c{g8ijo{-a0",
      },
      {
        ianaTimezone: "America/New_York",
        name: "(GMT-05:00) Eastern Time",
        data: "-8c{e5bvo{-6o{ecoa0{-8c{egs3o{-6o{eo4i0{-8c{es0jo{-6o{ezcy0{-8c{f38zo{-6o{fale0{-8c{fehfo{-6o{fltu0{-8c{fppvo{-6o{fx2a0{-8c{g163o{-6o{g8ii0{-8c",
      },
    ],
  },
  {
    countryCode: "UY",
    locales: [
      {
        ianaTimezone: "America/Montevideo",
        name: "(GMT-03:00) Montevideo",
        data: "-3c{e5bqo{-50",
      },
    ],
  },
  {
    countryCode: "UZ",
    locales: [
      {
        ianaTimezone: "Asia/Tashkent",
        name: "(GMT+05:00) Tashkent",
        data: "8c",
      },
    ],
  },
  {
    countryCode: "VA",
    locales: [
      {
        ianaTimezone: "Europe/Rome",
        name: "(GMT+01:00) Rome",
        data: "1o{e5yxo{3c{ecg9o{1o{eh7do{3c{enwho{1o{esfto{3c{ez4xo{1o{f3o9o{3c{faddo{1o{ff4ho{3c{fllto{1o{fqcxo{3c{fwu9o{1o{g1ldo{3c{g8aho{1o",
      },
    ],
  },
  {
    countryCode: "VC",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "VE",
    locales: [
      {
        ianaTimezone: "America/Caracas",
        name: "(GMT-04:00) Caracas",
        data: "-7i{eiajo{-6o",
      },
    ],
  },
  {
    countryCode: "VG",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "VI",
    locales: [
      {
        ianaTimezone: "America/Port_of_Spain",
        name: "(GMT-04:00) Port of Spain",
        data: "-6o",
      },
    ],
  },
  {
    countryCode: "VN",
    locales: [
      {
        ianaTimezone: "Asia/Saigon",
        name: "(GMT+07:00) Hanoi",
        data: "bo",
      },
    ],
  },
  {
    countryCode: "VU",
    locales: [
      {
        ianaTimezone: "Pacific/Efate",
        name: "(GMT+11:00) Efate",
        data: "ic",
      },
    ],
  },
  {
    countryCode: "WF",
    locales: [
      {
        ianaTimezone: "Pacific/Wallis",
        name: "(GMT+12:00) Wallis",
        data: "k0",
      },
    ],
  },
  {
    countryCode: "WS",
    locales: [
      {
        ianaTimezone: "Pacific/Apia",
        name: "(GMT+14:00) Apia",
        data: "nc{e667c{lo{ebknc{nc{ehenc{lo{emt3c{nc{esn3c{lo{ey1jc{nc{f3vjc{lo{f9hrc{nc{ffbrc{lo{fkq7c{nc{fqk7c{lo{fvync{nc{g1snc{lo{g773c{nc",
      },
    ],
  },
  {
    countryCode: "YE",
    locales: [
      {
        ianaTimezone: "Asia/Riyadh",
        name: "(GMT+03:00) Riyadh",
        data: "50",
      },
    ],
  },
  {
    countryCode: "YT",
    locales: [
      {
        ianaTimezone: "Africa/Nairobi",
        name: "(GMT+03:00) Nairobi",
        data: "50",
      },
    ],
  },
  {
    countryCode: "ZA",
    locales: [
      {
        ianaTimezone: "Africa/Johannesburg",
        name: "(GMT+02:00) Johannesburg",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "ZM",
    locales: [
      {
        ianaTimezone: "Africa/Maputo",
        name: "(GMT+02:00) Maputo",
        data: "3c",
      },
    ],
  },
  {
    countryCode: "ZW",
    locales: [
      {
        ianaTimezone: "Africa/Maputo",
        name: "(GMT+02:00) Maputo",
        data: "3c",
      },
    ],
  },
];
export const COUNTRY_NAMES = Object.keys(Country);
