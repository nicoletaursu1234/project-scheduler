import React, { useEffect, useReducer, ReactChild } from "react";
import {
  arraytizeFieldVal,
  getAvailableViews,
  getOneView,
...
const AppState = ({ initial, children }: AppProps) => {
  ...
  const triggerCalendarSidebar = (status: boolean | undefined) => {
    dispatch({ type: "triggerCalendarSidebar", payload: status });
  };
  ...
  const onDrop = async (
    ...
    if (resField) {
      ...
      if (isMultiple) {
        ...
        if (currentRes.includes(resVal)) {
          ...
          if (isEqual(droppedEvent.startDateTime, startTime)) {
            return;
          }
    ...
    const diff = differenceInMinutes(
      droppedEvent.endDateTime,
      droppedEvent.startDateTime
    );
    const updatedEvent: ProcessedEvent = {
      ...droppedEvent,
      startDateTime: startTime,
      endDateTime: addMinutes(startTime, diff),
      [resKey]: newResource || "",
    };
